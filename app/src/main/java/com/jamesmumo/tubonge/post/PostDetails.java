...
import com.jamesmumo.tubonge.adapter.AdapterComments;
import com.jamesmumo.tubonge.groups.ShareGroupActivity;
import com.jamesmumo.tubonge.model.ModelComments;
...
public class PostDetails extends AppCompatActivity implements View.OnClickListener {
        ...
        likeRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.child(postId).hasChild(userId)) {
                    ...
                    like_img.setImageResource(R.drawable.ic_hearted);
    ...
    private Uri saveImageToShare(Bitmap bitmap) {
        ...
        try {
            ...
            stream.flush();
            stream.close();
            uri = FileProvider.getUriForFile(this, "com.jamesmumo.tubonge.fileprovider", file);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == PERMISSION_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Alerter.create(PostDetails.this).setTitle("Successful").setIcon(R.drawable.ic_check_wt).setBackgroundColorRes(R.color.colorPrimaryDark).setDuration(10000).enableSwipeToDismiss().setTitleTypeface(Typeface.createFromAsset(getAssets(), "bold.ttf")).setTextTypeface(Typeface.createFromAsset(getAssets(), "med.ttf")).setText("Storage permission Allowed").show();
            } else {
                Alerter.create(PostDetails.this).setTitle("Error").setIcon(R.drawable.ic_error).setBackgroundColorRes(R.color.colorPrimaryDark).setDuration(10000).enableSwipeToDismiss().setTitleTypeface(Typeface.createFromAsset(getAssets(), "bold.ttf")).setTextTypeface(Typeface.createFromAsset(getAssets(), "med.ttf")).setText("Storage permission is required").show();
            }
        }
    }
    ...
    private void sendImage(Uri image_uri) {
        ...
        ref.putFile(image_uri).addOnSuccessListener(taskSnapshot -> {
            ...
            if (uriTask.isSuccessful()) {
                ...
                ref1.child(timeStamp).setValue(hashMap).addOnSuccessListener(aVoid -> {
                    textBox.setText("");
                    updateCommentCount();
                }).addOnFailureListener(e -> {
